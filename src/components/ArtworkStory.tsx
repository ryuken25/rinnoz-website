...
function ArtworkStoryDesktop({
  ...
  useGSAP(() => {
    ...
    gsap.set(cards, { autoAlpha: 0, yPercent: 14, xPercent: 5, scale: 0.9, rotate: 3, filter: 'blur(6px)', transformOrigin: '50% 50%' });
    ...
    cards.forEach((card, index) => {
      if (index === 0) return;
      tl.to(cards[index - 1], { autoAlpha: 0.14, yPercent: -10, xPercent: -8, scale: 0.84, rotate: -6, filter: 'blur(4px)', duration: 0.42, ease: 'power2.out' });
      tl.to(card, { autoAlpha: 1, yPercent: 0, xPercent: 0, scale: 1, rotate: index % 2 ? 1.5 : -1.5, filter: 'blur(0px)', duration: 0.52, ease: 'power2.out' }, '<0.08');
    });
      ...
      <div ref={pinRef} className="relative min-h-[calc(100svh-var(--nav-height,var(--navbar-height))-36px)] overflow-hidden bg-ink/20 py-[clamp(1rem,2vh,2rem)]">
        ...
        <div className="relative z-10 mx-auto grid min-h-[calc(100svh-var(--nav-height,var(--navbar-height))-48px)] w-full max-w-[1360px] grid-cols-[.86fr_1.14fr] items-center gap-[clamp(1.5rem,3vw,3rem)] px-[clamp(1.25rem,3vw,3.5rem)]">
          <aside className="story-panel max-h-[calc(100svh-var(--nav-height,var(--navbar-height))-48px)] overflow-hidden rounded-[1.8rem] border border-white/12 bg-ink/66 p-[clamp(1.1rem,1.7vw,1.65rem)] shadow-glow backdrop-blur-2xl">
            <p className="text-[clamp(.68rem,.72vw,.8rem)] font-black uppercase tracking-[.18em] text-blush">{storyEyebrow}</p>
            <motion.div key={`${active.id}-${meta.kicker}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24 }}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {meta.tags.map((tag) => (
                  <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[.72rem] font-bold">{tag}</span>
                ))}
              </div>
              <p className="mt-3 truncate text-[clamp(.78rem,.78vw,.92rem)] font-black text-lavender">
                {active.title} • {sourceLabel(active.source)}
              </p>
            </motion.div>
            <div className="story-steps mt-[clamp(.8rem,1.7vh,1.25rem)] space-y-2">
              {chapters.map((c, i) => (
                <button
                  type="button"
                  onClick={() => jumpTo(i)}
                ...
                >
                  <span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${activeIndex === i ? 'animate-pulse bg-blush' : 'bg-white/25'}`} />
                  <div className="min-w-0">
                    <p className="text-[clamp(.68rem,.68vw,.78rem)] font-black uppercase tracking-[.08em] text-blush">{chapterMeta[i]?.kicker}</p>
                    <p className="truncate text-[clamp(.78rem,.8vw,.95rem)] font-bold leading-snug">{chapterMeta[i]?.title}</p>
                  </div>
                </button>
              ))}
            </div>
            <div className="mt-[clamp(.8rem,1.8vh,1.25rem)] h-1.5 overflow-hidden rounded-full bg-white/10">
          ...
          <div className="story-art-frame relative flex min-h-[calc(100svh-var(--nav-height,var(--navbar-height))-72px)] items-center justify-center overflow-hidden rounded-[2rem] border border-white/12 bg-gradient-to-br from-white/[.09] to-white/[.03] p-[clamp(1rem,1.8vw,1.5rem)] shadow-atelier">
            {chapters.map((item, i) => (
              <article
                ...
                key={item.id}
                className="story-card absolute inset-[clamp(1rem,1.6vw,1.5rem)] grid place-items-center"
              >
                <button type="button" onClick={() => onSelect(item.id)} className="group relative flex h-full w-full items-center justify-center rounded-[1.6rem] bg-ink/42 p-[clamp(.7rem,1.2vw,1rem)]">
                  <Image
                    src={item.imageUrl}
                    alt={item.title}
                    width={item.width}
                    ...
                    className="h-auto max-h-[min(58svh,620px)] w-auto max-w-[min(680px,48vw)] rounded-[1.25rem] object-contain shadow-atelier"
                    priority={i < 1}
                  />
                  <span className="absolute left-4 top-4 rounded-full border border-white/15 bg-ink/70 px-3 py-1 text-[.72rem] font-black text-lavender backdrop-blur">{sourceLabel(item.source)}</span>
                  <span className="absolute bottom-4 left-4 right-4 rounded-2xl border border-white/12 bg-ink/72 p-3 text-left opacity-0 backdrop-blur-xl transition group-hover:opacity-100">
      ...
      {chapters.map((item, i) => (
        ...
        >
          ...
          <div className="p-3">
            ...
            <h3 className="mt-2 font-display text-[clamp(1.65rem,8vw,2.35rem)] leading-[1.05] tracking-[-.035em]">{chapterMeta[i]?.title}</h3>
            <p className="mt-2 text-[.95rem] leading-[1.65] text-cream/65">{chapterMeta[i]?.body}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {(chapterMeta[i]?.tags || []).map((tag) => (
                <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[.7rem] font-bold">{tag}</span>
              ))}
            </div>
          </div>
        </motion.button>
  ...
  return (
    <div className="mt-8">
      {isDesktop && !reduceMotion ? (
        <ArtworkStoryDesktop chapters={chapters} onSelect={onSelect} chapterMeta={chapterMeta} storyEyebrow={t('artworks.storyEyebrow')} />
      ) : (
