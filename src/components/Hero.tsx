...
export function Hero(){
  ...
  return <header id="home" className="relative flex min-h-screen items-center px-4 pb-16 pt-32">
  <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.02fr_.98fr]">
    ...
    <motion.div className="relative mx-auto w-full max-w-[620px]" animate={{y:[0,-10,0]}} transition={{repeat:Infinity,duration:7,ease:'easeInOut'}}>
      <div className="moon-orb absolute -right-8 -top-10 opacity-75"/>{minis[0] && <div className="absolute -left-7 top-12 rotate-[-9deg] rounded-3xl border border-white/15 bg-white/8 p-2 shadow-glow backdrop-blur-xl"><Image src={minis[0].imageUrl} alt={minis[0].title} width={minis[0].width} height={minis[0].height} className="h-28 w-28 rounded-2xl object-cover md:h-36 md:w-36"/></div>}
      ...
      {minis[1] && <div className="absolute -bottom-8 right-4 rotate-6 rounded-3xl border border-white/15 bg-white/8 p-2 shadow-blush backdrop-blur-xl"><Image src={minis[1].imageUrl} alt={minis[1].title} width={minis[1].width} height={minis[1].height} className="h-32 w-32 rounded-2xl object-cover md:h-44 md:w-44"/></div>}
    </motion.div>
  </div></header> }
